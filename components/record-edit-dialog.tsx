...
export function RecordEditDialog({ record, schema, open, onOpenChange, onSave }: RecordEditDialogProps) {
  ...
  const DRIVER_LINK = fields?.workSchedule.DRIVER_LINK_SYMMETRIC || ""
  const SENT_RESET = fields?.workSchedule.SENT_RESET || ""
  ...
  useEffect(() => {
    if (record) {
      ...
      setPreviousDriver(updatedFields[DRIVER_LINK])
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const currentDriver = formData[DRIVER_LINK]
