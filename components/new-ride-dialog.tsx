...
import { Plus, Loader2, Pencil, Upload, Calendar as CalendarIcon } from "lucide-react"
              ...
              <TabsContent value="details" className="space-y-4">
                <div className="space-y-1">
                    ...
                    <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                        ...
                        <Button variant={"outline"} className={cn("w-full justify-start text-right", showErrors && !date && "border-red-500")}>
                            <CalendarIcon className="ml-2 h-4 w-4" />
                            {date ? format(date, "PPP", { locale: he }) : "בחר תאריך"}
                        </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0" align="end">
                        <Calendar mode="single" selected={date} onSelect={(d) => {setDate(d); setIsCalendarOpen(false)}} initialFocus locale={he} dir="rtl" />
                        </PopoverContent>
                    </Popover>
                </div>
