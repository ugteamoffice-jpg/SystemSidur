...
export function RideDialog({ onRideSaved, initialData, triggerChild, open: controlledOpen, onOpenChange: setControlledOpen, defaultDate }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let attachmentData: any[] = [...existingAttachment]
      if (orderFormFile) {
        setIsUploading(true)
        const formData = new FormData()
        formData.append('file', orderFormFile)
        formData.append('tableId', 'tblUgEhLuyCwEK2yWG4')
        formData.append('fieldId', FIELDS.ORDER_FORM)
        ...
        attachmentData = [...attachmentData, { name: uploaded.name, token: uploaded.token, size: uploaded.size, mimetype: uploaded.mimetype }]
      ...
      const body: any = {
        fields: {
          ...
          [FIELDS.ORDER_FORM]: attachmentData.length > 0 ? attachmentData : null,
        }
      }
              ...
              <TabsContent value="details" className="space-y-4">
                ...
                <div className="space-y-2 pt-4 border-t mt-4">
                  ...
                  {existingAttachment.map((att: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-green-50 border border-green-200 rounded text-sm">
                      <FileText className="w-4 h-4 text-green-600 shrink-0" />
                      <span className="truncate flex-1">{att.name || 'קובץ מצורף'}</span>
                      <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 text-red-500 hover:text-red-700" onClick={() => setExistingAttachment(prev => prev.filter((_, idx) => idx !== i))}>
                        <X className="w-3 h-3" />
                      </Button>
                    </div>
                  ))}
                  ...
                  <Button type="button" variant="outline" size="sm" className="w-full" onClick={() => fileInputRef.current?.click()}>
                    ...
                    {orderFormFile || existingAttachment.length > 0 ? 'החלף / הוסף קובץ' : 'בחר קובץ'}
                  </Button>
                </div>
              </TabsContent>
