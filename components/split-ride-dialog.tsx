...
export function SplitRideDialog({ open, onOpenChange, record, onSplit }: any) {
  ...
  const [dateOpenOutbound, setDateOpenOutbound] = React.useState(false)
  const [dateOpenReturn, setDateOpenReturn] = React.useState(false)
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-1">
          <Label>תאריך</Label>
          <Popover open={isOutbound ? dateOpenOutbound : dateOpenReturn} onOpenChange={isOutbound ? setDateOpenOutbound : setDateOpenReturn}>
            ...
            <PopoverContent className="w-auto p-0" align="end">
              <Calendar mode="single" selected={trip.date} onSelect={(d) => { setTrip((p: any) => ({...p, date: d})); if (d) { isOutbound ? setDateOpenOutbound(false) : setDateOpenReturn(false) } }} initialFocus locale={he} dir="rtl" />
            </PopoverContent>
          </Popover>
        </div>
